...
import { X, AlertCircle, FileText, UploadCloud } from 'lucide-react';
import { NoticeFolder, Notice } from '../types';

interface FolderModalProps {
  isOpen: boolean;
  initialData?: NoticeFolder | null;
  onClose: () => void;
...
interface UploadNoticeModalProps {
    ...
    students: { id: string, name: string }[]; // To select visibility
    onClose: () => void;
...
export const UploadNoticeModal: React.FC<UploadNoticeModalProps> = ({ isOpen, folderId, initialData, students, onClose, onSave }) => {
    ...
    useEffect(() => {
      if (isOpen) {
        ...
        setFile(null); // File needs to be re-uploaded if changing unless we add specific logic to keep old
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        setErrorMsg(err.message || 'Erro ao processar edital.');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    return (
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
        ...
        <div className="relative bg-white dark:bg-dark-card w-full max-w-lg rounded-2xl shadow-xl p-6 border border-gray-100 dark:border-stone-700 animate-slide-up max-h-[90vh] overflow-y-auto">
          ...
          <h2 className="text-xl font-serif font-bold text-gray-800 dark:text-gray-100 mb-6">
            {initialData ? 'Editar Documento' : 'Novo Edital / Documento'}
          </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-surface dark:bg-dark-surface border border-gray-200 dark:border-stone-600 rounded-xl px-4 py-2.5 outline-none focus:border-secondary transition-colors dark:text-gray-100 placeholder:text-gray-400"
                placeholder="Ex: Edital de Abertura 2026"
                disabled={isLoading}
              />
            </div>
            ...
            <div className="pt-4 flex justify-end gap-3 mt-4">
              ...
              <button type="submit" disabled={isLoading} className="px-5 py-2.5 text-sm font-bold text-white bg-secondary hover:bg-[#6b5d52] rounded-xl transition-colors flex items-center justify-center min-w-[120px] disabled:opacity-50">
                {isLoading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : 'Salvar Edital'}
              </button>
            </div>
          </form>
        </div>
